...
@zedub.zed_cmd(pattern="(قلد|ازعاج)$")
async def echo(event):
    ...
    try:
        ...
        await edit_or_reply(
            zedubevent,
            "**- تم تفعيل امر القلد على هذا الشخص\nسيتم قلد جميع رسائله هنا**",
        )

@zedub.zed_cmd(pattern="(الغاء قلد|ايقاف القلد|ايقاف قلد|الغاء الازعاج|ايقاف الازعاج|ايقاف ازعاج|الغاء ازعاج)$")
async def echo(event):
    if event.reply_to_msg_id is None:
    ...
    if is_echo(chat_id, user_id):
        try:
            ...
            await edit_or_reply(event, "**⎉╎تم ايقاف القلد لهذا المستخدم**")
    else:
        await edit_or_reply(event, "**- لم يتم تفعيل القلد على هذا المستخدم اصلا**")
    ...
    if input_str:
        ...
        if len(lecho) == 0:
            return await edit_delete(
                event, "**- لم يتم تفعيل القلد حتى لمستخدم واحد اصلا.**"
            )
        try:
            remove_all_echos()
        except Exception as e:
            ...
            await edit_or_reply(
                event, "**⎉╎تم حذف قلد جميع المستخدمين في جميع الدردشات.**"
            )
    else:
...
async def echo(event):
    ...
    if input_str:
        ...
        group_chats = ""
        if len(lsts) <= 0:
            return await edit_or_reply(event, "**- لم يتم تفعيل القلد بالاصل ؟!**")
        for echos in lsts:
