...
async def amireallyalive(event):
    ...
    start = datetime.now()
    zedevent = await edit_or_reply(event, "**⎆ 𖡻جـاري .. فحـص البـوت الخـاص بك**")
    end = datetime.now()
    ms = (end - start).microseconds / 1000
    _, check_sgnirts = check_data_base_heal_th()
    Z_EMOJI = gvarstatus("ALIVE_EMOJI") or "𖤓 𖡻"
    ALIVE_TEXT = gvarstatus("ALIVE_TEXT") or "** ربوت ســهـۍ 𝗦𝗢𝗛𝗔 يعمـل  ..  جـيــداًً💞𓆩   **"
...
**{Z_EMOJI} إصـدار ســهـۍ   :** `{zdver}`
**{Z_EMOJI} إصـدار البـايثون :** `{pyver}`
**{Z_EMOJI} الوقـت :** `{uptime}`
**{Z_EMOJI} المسـتخدم:** {mention}
**{Z_EMOJI} قنـاة السـورس :** [نـحـن هنــا](https://t.me/mcsec7)"""
...
@zedub.zed_cmd(
    ...
    info={
        "header": "- لـ التحـقق من ان البـوت يعمـل جـيــداًً .. بخـاصيـة الانـلايـن ✓",
        "الاسـتخـدام": [
            "{tr}الفحص",
        ],
    },
...
async def amireallyialive(event):
    ...
    reply_to_id = await reply_id(event)
    Z_EMOJI = gvarstatus("ALIVE_EMOJI") or "𖤓 𖡻"
    zed_caption = "** ربوت ســهـۍ 𝗦𝗢𝗛𝗔 يعمـل  ..  جـيــداًً💞𓆩   **\n"
    zed_caption += f"**{Z_EMOJI} إصـدار التـيليثون :** `{version.__version__}\n`"
    zed_caption += f"**{Z_EMOJI} إصـدار ســهـۍ   :** `{zedversion}`\n"
