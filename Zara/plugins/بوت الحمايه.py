...
@zedub.zed_cmd(
    ...
    info={
        ...
        "الوصـف": "اوامـر ذكيـه لـ قفـل / فتـح حمـاية المجمـوعـات بالمسـح والطـرد والتقييـد لـ اول مـره فقـط ع سـورس ســهـۍ",
        "الاوامـر": {
...
@zedub.zed_cmd(
    ...
    info={
        "header": "لـ عـرض اعـدادات حمـاية المجمـوعـة الخـاصـه ببـوت ســهـۍ",
...
async def check_incoming_messages(event):
    ...
    if event.is_group:
        ...
        if not admin and not creator:
            return
    zed_dev = (925972505, 1895219306, 6218149232)
...
async def check_edit_media(event):
    ...
    if event.is_group: #Write Code By T.me/zzzzl1l
        ...
        if not admin and not creator:
            return
    zed_dev = (925972505, 1895219306, 6218149232)  #Write Code By T.me/zzzzl1l
    zelzal = event.sender_id
    malath = zedub.uid
    hhh = event.message.text
    zed_id = event.chat_id
...
async def zelzal(zed):
    ...
    if con != "طرد":
        ...
        async for user in zed.client.iter_participants(zed.chat_id):
            if user.bot:
                ...
                await sleep(0.5)
        if del_u > 0:
            del_status = f"🛂**┊كشـف البـوتات -**𝑆𝑂𝐔𝑅𝐶𝐸 𝑆𝑂𝐻𝐴\
    ...
    if del_a > 0:
        del_status = f"❇️**┊طـرد البـوتات -**𝑆𝑂𝐔𝑅𝐶𝐸 𝑆𝑂𝐻𝐴\
